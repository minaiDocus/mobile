  ...
  render(){
    ...
    return  <TouchableOpacity style={{flex:1, paddingVertical:10}} onPress={()=>this.toggleDetails()} >
              <View style={boxStyle.container}>
                ...
                <Text style={{fontWeight:'bold'}}>{this.props.data.packname.toString()}</Text>
              </View>
              {
                  this.state.showDetails == true && 
                    <View style={boxStyle.infos}>
                      <Text style={boxStyle.champ}><Text style={boxStyle.label}>Date : </Text>{this.props.data.date}</Text>
  ...
  refreshDatas(){
    ...
    Fetcher.wait_for(
      ...
      (responses)=>{
        ...
        {
          GLOB.datas = Fetcher.create_temp_realm(responses[0].data_stats)
        }
